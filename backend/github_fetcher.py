...
async def fetch_repo_engineers(
    ...
    async with httpx.AsyncClient(headers=_headers(token), timeout=30.0, follow_redirects=True) as client:
        ...
        # Wave 3: individual PR details for top merged PRs + review data — concurrent
        detail_tasks = []
        ...
        wave3 = await asyncio.gather(*detail_tasks, *review_tasks)
        detail_results = wave3[:len(detail_tasks)]
        review_results = wave3[len(detail_tasks):]
